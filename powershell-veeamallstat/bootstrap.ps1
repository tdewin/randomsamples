...
if($Env:PSModulePath -match ('{0}[^\;]+' -f ($Env:USERPROFILE -replace '\\','\\'))) {
	...
	dltextfile -uri $uri -fname $fname	
   	...
   	write-host "Installed $umodpath"
	
	$umodpath = ('{0}\veeamallstat' -f $Matches[0])
	$null = ni -Type Directory $umodpath -ErrorAction SilentlyContinue
	
	$uri = "https://raw.githubusercontent.com/tdewin/randomsamples/master/powershell-veeamallstat/veeamallstat/veeamallstat.psm1"
	$fname = ('{0}\veeamallstat.psm1' -f $umodpath)
	dltextfile -uri $uri -fname $fname

	$uri = "https://raw.githubusercontent.com/tdewin/randomsamples/master/powershell-veeamallstat/veeamallstat/veeamallstat.psd1"
	$fname = ('{0}\veeamallstat.psd1' -f $umodpath)
	dltextfile -uri $uri -fname $fname
